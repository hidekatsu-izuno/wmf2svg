...
public interface Gdi {
	...
	public void maskBlt(byte[] image, int dx, int dy, int dw, int dh, int sx, int sy, byte[] mask, int mx, int my,
			long rop);
	public default void maskBlt(byte[] image, int dx, int dy, int dw, int dh, int sx, int sy, byte[] mask, int mx,
			int my, long rop, Integer sourceBackgroundColor) {
		maskBlt(image, dx, dy, dw, dh, sx, sy, mask, mx, my, rop);
	}
