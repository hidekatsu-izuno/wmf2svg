...
public class EmfPlusParser implements EmfPlusConstants {
	public interface Handler {
		void handleEmfPlusRecord(int type, int flags, byte[] payload, boolean continuableObject, int totalObjectSize);
	}

	public static boolean isEmfPlusComment(byte[] data) {
		return data.length >= 4 && data[0] == 'E' && data[1] == 'M' && data[2] == 'F' && data[3] == '+';
	}
