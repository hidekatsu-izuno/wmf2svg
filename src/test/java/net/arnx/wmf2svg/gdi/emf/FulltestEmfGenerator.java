...
public class FulltestEmfGenerator implements EmfPlusConstants {
	...
	public static void main(String[] args) throws Exception {
		...
		panel(gdi, black, white, 20, 815, 520, 1145, "13 direct EMR records");
		gdi.selectObject(labelFont);
		gdi.setTextAlign(Gdi.TA_LEFT | Gdi.TA_TOP);
		...
		gdi.selectObject(black);
		gdi.rectangle(380, 895, 490, 1015);
		gdi.selectObject(white);
		...
		gdi.selectObject(labelFont);
		gdi.textOut(570, 855, ascii("EMR_GDICOMMENT containing EMF+ records."));
		gdi.textOut(570, 880, ascii("SVG output shows gradient, ellipse, line, text."));
		gdi.comment(createEmfPlusFulltestComment());

		panel(gdi, black, white, 1070, 815, 1575, 1145, "15 EMF+ compatibility");
		gdi.selectObject(labelFont);
		gdi.textOut(1095, 855, ascii("MS Paint may show only these GDI labels."));
		gdi.textOut(1095, 880, ascii("The EMF+ payload is still embedded in panel 14."));
		gdi.textOut(1095, 905, ascii("Use this project's SVG conversion to verify it."));
	...
	private static byte[] createEmfPlusFulltestComment() {
		...
		payload.reset();
		writeInt(payload, 0);
		writeInt(payload, EMF_PLUS_BRUSH_TYPE_LINEAR_GRADIENT);
		writeInt(payload, 0);
		writeInt(payload, EMF_PLUS_WRAP_MODE_TILE);
		writeFloat(payload, 380);
		writeFloat(payload, 610);
		writeFloat(payload, 120);
		writeFloat(payload, 70);
		writeInt(payload, 0xFFFFE070);
		writeInt(payload, 0xFF3050D0);
		writeInt(payload, 0);
		writeInt(payload, 0);
		writeEmfPlusRecord(comment, EMF_PLUS_OBJECT, 0x0101, payload.toByteArray());
		...
		writeFloat(payload, 120);
		writeFloat(payload, 70);
		writeEmfPlusRecord(comment, EMF_PLUS_FILL_RECTS, 0, payload.toByteArray());

		payload.reset();
		writeInt(payload, 0xFF66AAFF);
		writeFloat(payload, 535);
		writeFloat(payload, 615);
		writeFloat(payload, 95);
		writeFloat(payload, 65);
		writeEmfPlusRecord(comment, EMF_PLUS_FILL_ELLIPSE, EMF_PLUS_FLAG_SOLID_COLOR, payload.toByteArray());

		payload.reset();
		writeInt(payload, 0);
		writeInt(payload, 0);
		writeInt(payload, 0);
		writeInt(payload, EMF_PLUS_UNIT_PIXEL);
		writeFloat(payload, 4.5f);
		writeInt(payload, 0);
		writeInt(payload, EMF_PLUS_BRUSH_TYPE_SOLID_COLOR);
		writeInt(payload, 0xFFFF5533);
		writeEmfPlusRecord(comment, EMF_PLUS_OBJECT, 0x0202, payload.toByteArray());

		payload.reset();
		writeInt(payload, 3);
		writeFloat(payload, 380);
		writeFloat(payload, 720);
		writeFloat(payload, 445);
		writeFloat(payload, 685);
		writeFloat(payload, 510);
		writeFloat(payload, 735);
		writeEmfPlusRecord(comment, EMF_PLUS_DRAW_LINES, 0x0002, payload.toByteArray());

		payload.reset();
		writeInt(payload, 0);
		writeFloat(payload, 22);
		writeInt(payload, EMF_PLUS_UNIT_PIXEL);
		writeInt(payload, EMF_PLUS_FONT_STYLE_BOLD);
		writeInt(payload, 0);
		writeInt(payload, "Arial".length());
		writeUtf16Le(payload, "Arial");
		writeEmfPlusRecord(comment, EMF_PLUS_OBJECT, 0x0603, payload.toByteArray());

		payload.reset();
		writeInt(payload, 0xFF202020);
		writeInt(payload, 0);
		writeInt(payload, "EMF+ text".length());
		writeFloat(payload, 535);
		writeFloat(payload, 705);
	...
	private static ByteArrayOutputStream createEmfPlusComment() {
	...
	private static void writeEmfPlusRecord(ByteArrayOutputStream out, int type, int flags, byte[] data) {
		writeShort(out, type);
		writeShort(out, flags);
		writeInt(out, data.length + EMF_PLUS_HEADER_SIZE);
	...
	private static void writeUtf16Le(ByteArrayOutputStream out, String value) {
